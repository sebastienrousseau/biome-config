...
%global npm_bare      biome-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-biome-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Biome configuration rules for ultra-fast Rust-based linting and formatting

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/biome-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/biome-config is a shareable configuration preset with no runtime
...
install -pm 0644 biome.json %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
